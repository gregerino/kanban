...
export default function FilterBar({ filters, setFilters, people, labels, columns }) {
  return (
    <div className="flex flex-wrap items-center gap-2 px-6 py-3 bg-white border-b border-gray-100">
      ...
      <select value={filters.assignee} onChange={e => setFilters(f => ({ ...f, assignee: e.target.value }))} className="px-2 py-1 border border-gray-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-200">
      ...
      {(filters.assignee || filters.status || filters.priority || filters.label) && (
        <button onClick={() => setFilters({ assignee: '', status: '', priority: '', label: '' })} className="px-2 py-1 text-xs text-indigo-600 hover:text-indigo-800 font-medium">Clear</button>
      )}
    </div>
  );
}
