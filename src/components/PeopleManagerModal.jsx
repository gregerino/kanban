...
export default function PeopleManagerModal({ open, onClose, people, onSave }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Hantera team">
      <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
        {items.map(p => (
          <div key={p} className="flex items-center gap-2">
            ...
            <button onClick={() => remove(p)} className="text-xs text-red-500 hover:text-red-700">Ta bort</button>
          </div>
        ))}
      </div>
      <div className="flex gap-2">
        <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Personnamn..." className="flex-1 px-3 py-1.5 border border-gray-200 rounded-lg text-sm outline-none" onKeyDown={e => e.key === 'Enter' && add()} />
      ...
      <button onClick={save} className="w-full mt-4 px-4 py-2 bg-indigo-500 text-white rounded-lg text-sm font-medium hover:bg-indigo-600">Spara team</button>
    </Modal>
  );
}
