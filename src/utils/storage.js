...
function migrateBoard(d) {
  ...
  if (!d.whiteboard) d.whiteboard = { nodes: [], connections: [], labels: [], groups: [] };
  if (d.whiteboard && !d.whiteboard.labels) d.whiteboard.labels = [];
  if (d.whiteboard && !d.whiteboard.groups) d.whiteboard.groups = [];
...
export function createEmptyBoard(name) {
  return { id: uid(), name, icon: '', stories: [], tasks: [], labels: [], columns: DEFAULT_COLUMNS, customColors: [], backgroundImage: '', brainDumpLists: [], deadlineEnabled: false, whiteboardEnabled: false, whiteboard: { nodes: [], connections: [], labels: [], groups: [] } };
}
